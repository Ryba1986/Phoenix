...
namespace Phoenix.Services.Handlers.Devices.Queries
{
   ...
   {
      public async Task<IReadOnlyCollection<KeyValuePair<int, string>>> Handle(GetDeviceStopBitDictionaryQuery request, CancellationToken cancellationToken)
      {
         return await Task.FromResult(EnumExtensions.GetValues<SerialStopBits>().ToArray());
      }
   }
}
