...
namespace Phoenix.Services.Handlers.Devices.Queries
{
   ...
   {
      public async Task<IReadOnlyCollection<KeyValuePair<int, string>>> Handle(GetDeviceTypeDictionaryQuery request, CancellationToken cancellationToken)
      {
         return await Task.FromResult(EnumExtensions.GetValues<DeviceType>().ToArray());
      }
   }
}
